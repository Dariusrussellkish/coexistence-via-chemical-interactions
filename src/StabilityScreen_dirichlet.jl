...
function main(
    ...
    @sync @distributed for ns = 1:nSample
        ...
        cellRatioArray = reshape(rand(r[myid()], dirichlet), (1, :))
...
open("$basename.dat", "w") do io
    write(io, "$nCellType $nSample\n")
    for i = 1: 3
        for nct = 1:nCellType
            V0 = V0DT[i, nct, :]
            for v in V0
    ...
    for i = 1:nCellType
        for j = 1:nSample
            write(io, "$(initCellRatioArray[i,j]) ")
        end
        write(io, "\n")
    end
end
