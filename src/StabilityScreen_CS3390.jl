...
function main(
    ...
    NE0D = SharedArray{Float64,4}(
        (coarseness, coarseness, 3, nSample),
        init = zeros(coarseness, coarseness, 3, nSample),
    )
    ...
    for qp = 1:coarseness, qc = 1:coarseness
        ...
        @sync @distributed for ns = 1:nSample
            ...
            rIntMatA =
                R .*
                DistInteractionStrengthMT_PB(nCellType, nMediator, ri0, 0.5, r) # matrix of interaction coefficients, 50/50
            rIntMatB =
